...
void respondDisc(void)
{
	if (!(RdmFlags &(1<<MUTE_RDM)))
	{
        ...
        for (i=0; i<6; i++) {
            ...
            USART_transmit(current_byte|0xAA);
            USART_transmit(current_byte|0x55);
            checksum += current_byte;
        }
        USART_transmit(checksum>>8|0xAA);
        USART_transmit(checksum>>8|0x55);
        USART_transmit((checksum & 0xff)|0xAA);
        USART_transmit((checksum & 0xff)|0x55);
			...
			switch(swapInt(rdm->PID))
			{
				...
				case DEVICE_LABEL:
					if (rdm->Cmd == SET_CMD)					//set new device label
					{
						...
						eeprom_write_byte((uint8_t*)EEPROM_LABEL_LEN, rdm->PDLen);
						eeprom_write_block(rdm->Data, (uint16_t*)EEPROM_LABEL, rdm->PDLen);
					}
					else
					{
						rdm->PDLen= eeprom_read_byte((uint8_t*)EEPROM_LABEL_LEN);
						eeprom_read_block(rdm->Data, (uint16_t*)EEPROM_LABEL, rdm->PDLen); //response with device label
					}
					respondMsg();
					break;
